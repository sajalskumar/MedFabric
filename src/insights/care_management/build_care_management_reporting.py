...
#
# Purpose:
#     Builds care management reporting summaries from Layer 2 Care Management
#     outputs.
#
# Business Context:
#     Care Management Reporting helps executives and operational leaders
#     understand:
#
#         - care program member volume
#         - open case volume
#         - transitions-of-care member volume
#         - outreach tracking volume
#         - care management reporting readiness
#
# Architectural Rule:
#     Care Management Reporting consumes Layer 2 Analytics Platform outputs only.
#
#     This file does NOT:
#         - read raw, bronze, silver, gold, feature store, or modeling data
#         - assign care programs
#         - build case management worklists
#         - calculate disease management rules
#         - update outreach workflow status
#
# Inputs:
#     config/insights/insights.yaml
#     data/analytics_platform/care_management/*.parquet
#
# Outputs:
#     data/insights/care_management/care_management_reporting_summary.parquet
#
# Run:
#     python -m src.insights.care_management.build_care_management_reporting
#
###############################################################################
...
DEFAULT_DOMAIN_NAME = "Care Management Reporting"
DOMAIN_SECTION = "care_management_reporting"

PLATFORM_SECTION = "insights"

LOGGER_NAME = "medfabric.insights.care_management"
...
def validate_config(config: Dict[str, Any]) -> None:
    ...
    Purpose
    -------
    Validate required Care Management Reporting configuration sections.
    ...
    Notes
    -----
    This function validates only the Care Management Reporting domain contract.
    ...
    care_management_config = config.get("care_management_reporting", {})

    for subsection in ["source_datasets", "metrics"]:
        if subsection not in care_management_config:
            missing_sections.append(f"care_management_reporting.{subsection}")

    if missing_sections:
        raise PipelineError(
            "Care Management Reporting configuration validation failed. "
...
def initialize_runtime(config_path: str) -> InsightsDomainRuntime:
    ...
    Purpose
    -------
    Initialize Care Management Reporting runtime.
    ...
    -------
    InsightsDomainRuntime
        Initialized runtime for Care Management Reporting.
    ...
    context = create_pipeline_context(
        pipeline_name="Layer 3 - Care Management Reporting"
    )
    ...
    add_success_audit(
        ...
        message="Care Management Reporting runtime initialized successfully.",
...
def calculate_metric_value(
    ...
    Purpose
    -------
    Calculate a single configured care management reporting metric.
    ...
    if not column:
        raise ValidationError(
            f"Care Management metric '{metric_name}' requires a configured "
            f"column for calculation_type '{calculation_type}'."
        )

    if column not in dataframe.columns:
        raise ValidationError(
            f"Care Management metric '{metric_name}' source column missing: {column}"
        )
    ...
    raise ValidationError(
        f"Unsupported Care Management metric calculation_type: {calculation_type}"
    )


def build_care_management_metric_row(
    ...
    Purpose
    -------
    Build one standardized care management reporting metric row.

    Parameters
    ----------
    runtime:
        Care Management Reporting runtime.
    ...
    -------
    dict
        Care Management reporting metric row.
    ...
    return {
        "reporting_domain": "care_management_reporting",
        "metric_key": metric_name,
        "metric_label": metric_config.get("label", metric_name),
        "metric_category": metric_config.get("category", "Care Management"),
        "calculation_type": metric_config.get("calculation_type"),
        ...
        "insights_asset_name": "care_management_reporting_summary",
        "built_at_utc": utc_now().isoformat(),
    }


###############################################################################
# Care Management Reporting Summary
###############################################################################

def build_care_management_reporting_summary(
    runtime: InsightsDomainRuntime,
    ...
    Purpose
    -------
    Build care management reporting summary.
    ...
    datasets:
        Loaded Layer 2 Care Management datasets.
    ...
    config = runtime.config.get("care_management_reporting", {})

    if not bool(config.get("enabled", True)):
        logger.info("SKIP: Care Management Reporting disabled")
        return pd.DataFrame()

    rows: List[Dict[str, Any]] = []

    logger.info("START: Build care management reporting summary")
    ...
    for metric_name, metric_config in config.get("metrics", {}).items():
        ...
        add_metric_rule_record(
            ...
            description=f"Care Management reporting metric: {metric_name}",
        )
        ...
        if source_dataset not in datasets:
            warn_dataset_missing(
                ...
                source_layer="Layer 2 - Care Management",
            )
        ...
        rows.append(
            build_care_management_metric_row(
                runtime=runtime,
                metric_name=metric_name,
                metric_config=metric_config,
                metric_value=metric_value,
        ...
        logger.info(
            "Built care management metric: %s | Value: %s | Source: %s",
            metric_name,
            metric_value,
            source_dataset,
        )
    ...
    add_rule_record(
        runtime=runtime,
        rule_group="care_management_reporting",
        rule_name="care_management_reporting_summary_assembly",
        rule_type="reporting_summary",
        description=(
            "Builds care management reporting summary from Care Management "
            "Analytics Platform outputs."
        ),
        source_dataset="multiple_care_management_outputs",
        source_layer="Layer 2 - Care Management",
        rule_config=config,
    )
    ...
    register_output_asset(
        runtime=runtime,
        dataset_name="care_management_reporting_summary",
        dataframe=output_df,
        dataset_type="care_management_reporting_output",
        message="Care Management reporting summary built successfully.",
        source_layer="Layer 2 - Care Management",
        source_dataset="multiple_care_management_outputs",
    )

    logger.info(
        "COMPLETE: Build care management reporting summary | Rows: %s",
        len(output_df),
    )
...
def build_care_management_reporting(
    config_path: str = DEFAULT_CONFIG_PATH,
) -> InsightsBuildResult:
    """
    Purpose
    -------
    Build the Care Management Reporting domain.
    ...
    -------
    InsightsBuildResult
        Standard build result for Care Management Reporting.
    ...
    Notes
    -----
    This function performs the complete Care Management Reporting workflow:
        - initialize runtime
        - load configured Layer 2 Care Management inputs
        - calculate care management reporting metrics
        - write output
        - write metadata
        - write audit outputs
    """
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info("MedFabric Care Management Reporting started")
        logger.info("=" * 80)
        ...
        care_management_reporting_summary = build_care_management_reporting_summary(
            runtime=runtime,
            datasets=datasets,
        )

        output_assets: Dict[str, pd.DataFrame] = {
            "care_management_reporting_summary": care_management_reporting_summary,
        }
        ...
        add_success_audit(
            runtime=runtime,
            step_name="build_care_management_reporting",
            message="Care Management Reporting completed successfully.",
            row_count=sum(len(dataframe) for dataframe in output_assets.values()),
            source_layer="Layer 3 - Insights",
            source_dataset="care_management_reporting_outputs",
        )
        ...
        logger.info("=" * 80)
        logger.info("MedFabric Care Management Reporting completed successfully")
        logger.info("=" * 80)

        return InsightsBuildResult(
            name="care_management_reporting",
        ...
        if runtime is not None:
            ...
            logger.error("=" * 80)
            logger.error("Care Management Reporting failed")
            ...
            add_failed_audit(
                ...
                message=str(exc),
                source_layer="Layer 3 - Insights",
                source_dataset="care_management_reporting",
            )
            ...
            try:
                ...
                logger.error(
                    "Failed to write Care Management Reporting audit outputs: %s",
                    audit_exc,
                )
...
def main() -> None:
    ...
    Purpose
    -------
    Command-line entry point for Care Management Reporting.
    ...
    result = build_care_management_reporting(config_path=config_path)

    if result.status == STATUS_SUCCESS:
        print(result.message)
        return

    print(f"Care Management Reporting failed: {result.message}")
    sys.exit(1)
