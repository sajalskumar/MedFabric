...
#
# Purpose:
#     Builds population reporting summaries from Layer 2 Population Health and
#     Predictive Analytics outputs.
#
# Business Context:
#     Population reporting helps executives and analysts understand:
#
#         - population member coverage
#         - population segment availability
#         - members with prediction summaries
#         - average prediction score across the population
#         - population reporting readiness
#
# Architectural Rule:
#     Population Reporting consumes Layer 2 Analytics Platform outputs only.
#
#     This file does NOT:
#         - read raw, bronze, silver, gold, feature store, or modeling data
#         - build population segmentation logic
#         - train predictive models
#         - score members
#
# Inputs:
#     config/insights/insights.yaml
#     data/analytics_platform/population_health/*.parquet
#     data/analytics_platform/predictive_analytics/*.parquet
#
# Outputs:
#     data/insights/population/population_reporting_summary.parquet
#
# Run:
#     python -m src.insights.population.build_population_reporting
#
###############################################################################
...
DEFAULT_DOMAIN_NAME = "Population Reporting"
DOMAIN_SECTION = "population_reporting"

PLATFORM_SECTION = "insights"

LOGGER_NAME = "medfabric.insights.population"
...
def validate_config(config: Dict[str, Any]) -> None:
    ...
    Purpose
    -------
    Validate required Population Reporting configuration sections.
    ...
    Notes
    -----
    This function validates only the Population Reporting domain contract.
    ...
    population_config = config.get("population_reporting", {})

    for subsection in ["source_datasets", "metrics"]:
        if subsection not in population_config:
            missing_sections.append(f"population_reporting.{subsection}")

    if missing_sections:
        raise PipelineError(
            "Population Reporting configuration validation failed. "
...
def initialize_runtime(config_path: str) -> InsightsDomainRuntime:
    ...
    Purpose
    -------
    Initialize Population Reporting runtime.
    ...
    -------
    InsightsDomainRuntime
        Initialized runtime for Population Reporting.
    ...
    Notes
    -----
    This follows the same MedFabric shared runtime pattern as other Layer 3
    reporting domains.
    """
    ...
    context = create_pipeline_context(
        pipeline_name="Layer 3 - Population Reporting"
    )
    ...
    add_success_audit(
        ...
        message="Population Reporting runtime initialized successfully.",
...
def calculate_metric_value(
    ...
    Purpose
    -------
    Calculate a single configured population reporting metric.
    ...
    if not column:
        raise ValidationError(
            f"Population metric '{metric_name}' requires a configured column "
            f"for calculation_type '{calculation_type}'."
        )

    if column not in dataframe.columns:
        raise ValidationError(
            f"Population metric '{metric_name}' source column missing: {column}"
        )
    ...
    raise ValidationError(
        f"Unsupported population metric calculation_type: {calculation_type}"
    )


def build_population_metric_row(
    ...
    Purpose
    -------
    Build one standardized population reporting metric row.

    Parameters
    ----------
    runtime:
        Population Reporting runtime.
    ...
    -------
    dict
        Population reporting metric row.
    ...
    Notes
    -----
    The output schema is intentionally aligned with other Insights reporting
    summaries so dashboards can combine domains consistently.
    """

    return {
        "reporting_domain": "population_reporting",
        "metric_key": metric_name,
        "metric_label": metric_config.get("label", metric_name),
        "metric_category": metric_config.get("category", "Population"),
        "calculation_type": metric_config.get("calculation_type"),
        ...
        "insights_asset_name": "population_reporting_summary",
        "built_at_utc": utc_now().isoformat(),
    }


###############################################################################
# Population Reporting Summary
###############################################################################

def build_population_reporting_summary(
    runtime: InsightsDomainRuntime,
    ...
    Purpose
    -------
    Build population reporting summary.
    ...
    config = runtime.config.get("population_reporting", {})

    if not bool(config.get("enabled", True)):
        logger.info("SKIP: Population Reporting disabled")
        return pd.DataFrame()

    rows: List[Dict[str, Any]] = []

    logger.info("START: Build population reporting summary")
    ...
    for metric_name, metric_config in config.get("metrics", {}).items():
        ...
        add_metric_rule_record(
            ...
            description=f"Population reporting metric: {metric_name}",
        )
        ...
        logger.info(
            "Built population metric: %s | Value: %s | Source: %s",
            metric_name,
            metric_value,
            source_dataset,
        )
    ...
    add_rule_record(
        runtime=runtime,
        rule_group="population_reporting",
        rule_name="population_reporting_summary_assembly",
        rule_type="reporting_summary",
        description=(
            "Builds population reporting summary from Population Health and "
            "Predictive Analytics outputs."
        ),
    ...
    register_output_asset(
        runtime=runtime,
        dataset_name="population_reporting_summary",
        dataframe=output_df,
        dataset_type="population_reporting_output",
        message="Population reporting summary built successfully.",
        source_layer="Layer 2 - Analytics Platform",
        source_dataset="multiple_layer_2_outputs",
    )

    logger.info("COMPLETE: Build population reporting summary | Rows: %s", len(output_df))
...
def build_population_reporting(
    config_path: str = DEFAULT_CONFIG_PATH,
) -> InsightsBuildResult:
    """
    Purpose
    -------
    Build the Population Reporting domain.
    ...
    -------
    InsightsBuildResult
        Standard build result for Population Reporting.
    ...
    Notes
    -----
    This function performs the complete Population Reporting workflow:
        - initialize runtime
        - load configured Layer 2 inputs
        - calculate population reporting metrics
        - write output
        - write metadata
        - write audit outputs
    """
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info("MedFabric Population Reporting started")
        logger.info("=" * 80)
        ...
        population_reporting_summary = build_population_reporting_summary(
            runtime=runtime,
            datasets=datasets,
        )

        output_assets: Dict[str, pd.DataFrame] = {
            "population_reporting_summary": population_reporting_summary,
        }
        ...
        add_success_audit(
            runtime=runtime,
            step_name="build_population_reporting",
            message="Population Reporting completed successfully.",
            row_count=sum(len(dataframe) for dataframe in output_assets.values()),
            source_layer="Layer 3 - Insights",
            source_dataset="population_reporting_outputs",
        )
        ...
        logger.info("=" * 80)
        logger.info("MedFabric Population Reporting completed successfully")
        logger.info("=" * 80)

        return InsightsBuildResult(
            name="population_reporting",
        ...
        if runtime is not None:
            ...
            logger.error("=" * 80)
            logger.error("Population Reporting failed")
            ...
            add_failed_audit(
                ...
                message=str(exc),
                source_layer="Layer 3 - Insights",
                source_dataset="population_reporting",
            )
            ...
            try:
                ...
                logger.error(
                    "Failed to write Population Reporting audit outputs: %s",
                    audit_exc,
                )
...
def main() -> None:
    ...
    Purpose
    -------
    Command-line entry point for Population Reporting.
    ...
    result = build_population_reporting(config_path=config_path)

    if result.status == STATUS_SUCCESS:
        print(result.message)
        return

    print(f"Population Reporting failed: {result.message}")
    sys.exit(1)
