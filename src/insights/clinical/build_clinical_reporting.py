...
#
# Purpose:
#     Builds clinical reporting summaries from Layer 2 Clinical Analytics and
#     Care Management outputs.
#
# Business Context:
#     Clinical reporting helps summarize:
#
#         - members with clinical conditions
#         - disease-management candidates
#         - clinical registry coverage
#         - clinical reporting availability
#
# Architectural Rule:
#     Clinical Reporting consumes Layer 2 Analytics Platform outputs only.
#
#     This file does NOT:
#         - read raw, bronze, silver, gold, feature store, or modeling data
#         - build condition registries
#         - assign disease programs
#         - train or score predictive models
#
# Inputs:
#     config/insights/insights.yaml
#     data/analytics_platform/clinical_analytics/*.parquet
#     data/analytics_platform/care_management/*.parquet
#
# Outputs:
#     data/insights/clinical/clinical_reporting_summary.parquet
#
# Run:
#     python -m src.insights.clinical.build_clinical_reporting
#
###############################################################################
...
DEFAULT_DOMAIN_NAME = "Clinical Reporting"
DOMAIN_SECTION = "clinical_reporting"

PLATFORM_SECTION = "insights"

LOGGER_NAME = "medfabric.insights.clinical"
...
def validate_config(config: Dict[str, Any]) -> None:
    ...
    Purpose
    -------
    Validate required Clinical Reporting configuration sections.
    ...
    Notes
    -----
    This function validates only the Clinical Reporting domain contract.
    """
    ...
    clinical_config = config.get("clinical_reporting", {})

    for subsection in ["source_datasets", "metrics"]:
        if subsection not in clinical_config:
            missing_sections.append(f"clinical_reporting.{subsection}")

    if missing_sections:
        raise PipelineError(
            "Clinical Reporting configuration validation failed. "
...
def initialize_runtime(config_path: str) -> InsightsDomainRuntime:
    ...
    Purpose
    -------
    Initialize Clinical Reporting runtime.
    ...
    -------
    InsightsDomainRuntime
        Initialized runtime for Clinical Reporting.
    ...
    context = create_pipeline_context(
        pipeline_name="Layer 3 - Clinical Reporting"
    )
    ...
    add_success_audit(
        ...
        message="Clinical Reporting runtime initialized successfully.",
...
def calculate_metric_value(
    ...
    Purpose
    -------
    Calculate a single configured clinical reporting metric.
    ...
    if not column:
        raise ValidationError(
            f"Clinical metric '{metric_name}' requires a configured column "
            f"for calculation_type '{calculation_type}'."
        )

    if column not in dataframe.columns:
        raise ValidationError(
            f"Clinical metric '{metric_name}' source column missing: {column}"
        )
    ...
    raise ValidationError(
        f"Unsupported clinical metric calculation_type: {calculation_type}"
    )


def build_clinical_metric_row(
    ...
    Purpose
    -------
    Build one standardized clinical reporting metric row.

    Parameters
    ----------
    runtime:
        Clinical Reporting runtime.
    ...
    return {
        "reporting_domain": "clinical_reporting",
        "metric_key": metric_name,
        "metric_label": metric_config.get("label", metric_name),
        "metric_category": metric_config.get("category", "Clinical"),
        "calculation_type": metric_config.get("calculation_type"),
        ...
        "insights_asset_name": "clinical_reporting_summary",
...
def build_clinical_reporting_summary(
    runtime: InsightsDomainRuntime,
    ...
    Purpose
    -------
    Build clinical reporting summary.
    ...
    MISSING_SOURCE_DATASET status.
    """

    logger = runtime.get_logger(LOGGER_NAME)
    config = runtime.config.get("clinical_reporting", {})

    if not bool(config.get("enabled", True)):
        logger.info("SKIP: Clinical Reporting disabled")
        return pd.DataFrame()

    rows: List[Dict[str, Any]] = []

    logger.info("START: Build clinical reporting summary")
    ...
    for metric_name, metric_config in config.get("metrics", {}).items():
        ...
        add_metric_rule_record(
            ...
            description=f"Clinical reporting metric: {metric_name}",
        )
        ...
        logger.info(
            "Built clinical metric: %s | Value: %s | Source: %s",
            metric_name,
            metric_value,
            source_dataset,
        )
    ...
    add_rule_record(
        runtime=runtime,
        rule_group="clinical_reporting",
        rule_name="clinical_reporting_summary_assembly",
        rule_type="reporting_summary",
        description=(
            "Builds clinical reporting summary from Clinical Analytics and "
            "Care Management outputs."
        ),
    ...
    register_output_asset(
        runtime=runtime,
        dataset_name="clinical_reporting_summary",
        dataframe=output_df,
        dataset_type="clinical_reporting_output",
        message="Clinical reporting summary built successfully.",
        source_layer="Layer 2 - Analytics Platform",
        source_dataset="multiple_layer_2_outputs",
    )

    logger.info("COMPLETE: Build clinical reporting summary | Rows: %s", len(output_df))
...
def build_clinical_reporting(
    config_path: str = DEFAULT_CONFIG_PATH,
) -> InsightsBuildResult:
    """
    Purpose
    -------
    Build the Clinical Reporting domain.
    ...
    -------
    InsightsBuildResult
        Standard build result for Clinical Reporting.
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info("MedFabric Clinical Reporting started")
        logger.info("=" * 80)
        ...
        clinical_reporting_summary = build_clinical_reporting_summary(
            runtime=runtime,
            datasets=datasets,
        )

        output_assets: Dict[str, pd.DataFrame] = {
            "clinical_reporting_summary": clinical_reporting_summary,
        }
        ...
        add_success_audit(
            runtime=runtime,
            step_name="build_clinical_reporting",
            message="Clinical Reporting completed successfully.",
            row_count=sum(len(dataframe) for dataframe in output_assets.values()),
            source_layer="Layer 3 - Insights",
            source_dataset="clinical_reporting_outputs",
        )
        ...
        logger.info("=" * 80)
        logger.info("MedFabric Clinical Reporting completed successfully")
        logger.info("=" * 80)
        ...
        if runtime is not None:
            ...
            logger.error("=" * 80)
            logger.error("Clinical Reporting failed")
            ...
            add_failed_audit(
                ...
                message=str(exc),
                source_layer="Layer 3 - Insights",
                source_dataset="clinical_reporting",
            )
            ...
            try:
                ...
                logger.error(
                    "Failed to write Clinical Reporting audit outputs: %s",
                    audit_exc,
                )
...
def main() -> None:
    ...
    Purpose
    -------
    Command-line entry point for Clinical Reporting.
    ...
    result = build_clinical_reporting(config_path=config_path)

    if result.status == STATUS_SUCCESS:
        print(result.message)
        return

    print(f"Clinical Reporting failed: {result.message}")
    sys.exit(1)
