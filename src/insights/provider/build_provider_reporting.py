...
#
# Purpose:
#     Builds provider reporting summaries from Layer 2 Provider Analytics outputs.
#
# Business Context:
#     Provider reporting helps executives and analysts understand:
#
#         - provider count
#         - high-performing provider count
#         - provider performance reporting availability
#         - provider cost reporting availability
#         - provider utilization reporting availability
#         - provider network and specialty reporting coverage
#
# Architectural Rule:
#     Provider Reporting consumes Layer 2 Analytics Platform outputs only.
#
#     This file does NOT:
#         - read raw, bronze, silver, gold, feature store, or modeling data
#         - build provider attribution
#         - calculate provider analytics business rules
...
#
# Outputs:
#     data/insights/provider/provider_reporting_summary.parquet
#
# Run:
#     python -m src.insights.provider.build_provider_reporting
#
###############################################################################
...
DEFAULT_DOMAIN_NAME = "Provider Reporting"
DOMAIN_SECTION = "provider_reporting"

PLATFORM_SECTION = "insights"

LOGGER_NAME = "medfabric.insights.provider"
...
def validate_config(config: Dict[str, Any]) -> None:
    ...
    Purpose
    -------
    Validate required Provider Reporting configuration sections.
    ...
    Notes
    -----
    This function validates only the Provider Reporting domain contract.
    ...
    provider_config = config.get("provider_reporting", {})

    for subsection in ["source_datasets", "metrics"]:
        if subsection not in provider_config:
            missing_sections.append(f"provider_reporting.{subsection}")

    if missing_sections:
        raise PipelineError(
            "Provider Reporting configuration validation failed. "
...
def initialize_runtime(config_path: str) -> InsightsDomainRuntime:
    ...
    Purpose
    -------
    Initialize Provider Reporting runtime.
    ...
    -------
    InsightsDomainRuntime
        Initialized runtime for Provider Reporting.
    ...
    context = create_pipeline_context(
        pipeline_name="Layer 3 - Provider Reporting"
    )
    ...
    add_success_audit(
        ...
        message="Provider Reporting runtime initialized successfully.",
...
def calculate_metric_value(
    ...
    Purpose
    -------
    Calculate a single configured provider reporting metric.
    ...
    if not column:
        raise ValidationError(
            f"Provider metric '{metric_name}' requires a configured column "
            f"for calculation_type '{calculation_type}'."
        )

    if column not in dataframe.columns:
        raise ValidationError(
            f"Provider metric '{metric_name}' source column missing: {column}"
        )
...
def build_provider_metric_row(
    ...
    Purpose
    -------
    Build one standardized provider reporting metric row.

    Parameters
    ----------
    runtime:
        Provider Reporting runtime.
    ...
    Notes
    -----
    The output schema is aligned with other Insights reporting summaries so
    reporting domains can be combined consistently.
    """

    return {
        "reporting_domain": "provider_reporting",
        "metric_key": metric_name,
        "metric_label": metric_config.get("label", metric_name),
        "metric_category": metric_config.get("category", "Provider"),
        "calculation_type": metric_config.get("calculation_type"),
        ...
        "insights_asset_name": "provider_reporting_summary",
        "built_at_utc": utc_now().isoformat(),
    }


###############################################################################
# Provider Reporting Summary
###############################################################################

def build_provider_reporting_summary(
    runtime: InsightsDomainRuntime,
    ...
    Purpose
    -------
    Build provider reporting summary.
    ...
    datasets:
        Loaded Layer 2 Provider Analytics datasets.
    ...
    config = runtime.config.get("provider_reporting", {})

    if not bool(config.get("enabled", True)):
        logger.info("SKIP: Provider Reporting disabled")
        return pd.DataFrame()

    rows: List[Dict[str, Any]] = []

    logger.info("START: Build provider reporting summary")
    ...
    for metric_name, metric_config in config.get("metrics", {}).items():
        ...
        add_metric_rule_record(
            ...
            description=f"Provider reporting metric: {metric_name}",
        )
        ...
        if source_dataset not in datasets:
            warn_dataset_missing(
                ...
                source_layer="Layer 2 - Provider Analytics",
            )
        ...
        logger.info(
            "Built provider metric: %s | Value: %s | Source: %s",
            metric_name,
            metric_value,
            source_dataset,
        )
    ...
    add_rule_record(
        runtime=runtime,
        rule_group="provider_reporting",
        rule_name="provider_reporting_summary_assembly",
        rule_type="reporting_summary",
        description=(
            "Builds provider reporting summary from Provider Analytics outputs."
        ),
        source_dataset="multiple_provider_analytics_outputs",
        source_layer="Layer 2 - Provider Analytics",
        rule_config=config,
    )
    ...
    register_output_asset(
        runtime=runtime,
        dataset_name="provider_reporting_summary",
        dataframe=output_df,
        dataset_type="provider_reporting_output",
        message="Provider reporting summary built successfully.",
        source_layer="Layer 2 - Provider Analytics",
        source_dataset="multiple_provider_analytics_outputs",
    )

    logger.info("COMPLETE: Build provider reporting summary | Rows: %s", len(output_df))
...
def build_provider_reporting(
    config_path: str = DEFAULT_CONFIG_PATH,
) -> InsightsBuildResult:
    """
    Purpose
    -------
    Build the Provider Reporting domain.
    ...
    -------
    InsightsBuildResult
        Standard build result for Provider Reporting.
    ...
    Notes
    -----
    This function performs the complete Provider Reporting workflow:
        - initialize runtime
        - load configured Layer 2 Provider Analytics inputs
        - calculate provider reporting metrics
        - write output
        - write metadata
        - write audit outputs
    """
    ...
    try:
        ...
        logger.info("=" * 80)
        logger.info("MedFabric Provider Reporting started")
        logger.info("=" * 80)
        ...
        provider_reporting_summary = build_provider_reporting_summary(
            runtime=runtime,
            datasets=datasets,
        )

        output_assets: Dict[str, pd.DataFrame] = {
            "provider_reporting_summary": provider_reporting_summary,
        }
        ...
        add_success_audit(
            runtime=runtime,
            step_name="build_provider_reporting",
            message="Provider Reporting completed successfully.",
            row_count=sum(len(dataframe) for dataframe in output_assets.values()),
            source_layer="Layer 3 - Insights",
            source_dataset="provider_reporting_outputs",
        )
        ...
        logger.info("=" * 80)
        logger.info("MedFabric Provider Reporting completed successfully")
        logger.info("=" * 80)

        return InsightsBuildResult(
            name="provider_reporting",
        ...
        if runtime is not None:
            ...
            logger.error("=" * 80)
            logger.error("Provider Reporting failed")
            ...
            add_failed_audit(
                ...
                message=str(exc),
                source_layer="Layer 3 - Insights",
                source_dataset="provider_reporting",
            )
            ...
            try:
                ...
                logger.error(
                    "Failed to write Provider Reporting audit outputs: %s",
                    audit_exc,
                )
...
def main() -> None:
    ...
    Purpose
    -------
    Command-line entry point for Provider Reporting.
    ...
    result = build_provider_reporting(config_path=config_path)

    if result.status == STATUS_SUCCESS:
        print(result.message)
        return

    print(f"Provider Reporting failed: {result.message}")
    sys.exit(1)
